        ...
        {Array.from({ length: 18 }).map((_, a) => (
          <button
            ...
            className={`flex w-full snap-center items-center justify-center rounded-md text-sm transition ${
              a === value ? "bg-[var(--green)]/10 font-black text-[var(--green)]" : "text-[var(--muted)] hover:text-[var(--ink)]"
            }`}
          >
            {a}
          </button>
...
function AgePicker({ value, onChange }: { value: number; onChange: (n: number) => void }) {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-center gap-1 rounded-lg border border-[var(--line)] py-1.5 text-sm font-bold transition hover:border-[var(--green)]/50"
      >
        {value}
  ...
  return (
    <div>
      <div className="mb-2 flex items-center justify-between">
        <span className="text-sm font-bold">תקציב ללילה</span>
        <span className="text-xs font-bold text-[var(--green)]">${min} - ${max}{max === HI ? "+" : ""}</span>
      </div>
      <div dir="ltr">
      ...
      <div ref={ref} onScroll={update} className="no-scrollbar max-h-72 overflow-y-auto p-2">
        <button onClick={onAll} className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-right transition hover:bg-[var(--green)]/[0.07]">
          ...
          <span className="min-w-0">
            <span className="block text-sm font-bold">כל מקום</span>
        ...
        {results.map((p, i) => (
          ...
          >
            ...
            <span className="min-w-0">
              <span className="block truncate text-sm font-bold">{p.he}</span>
  ...
  return (
    <main className="min-h-screen">
      ...
      <nav className="sticky top-0 z-30 border-b border-[var(--line)] bg-[var(--bg)]/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-5 text-sm text-[var(--ink)]/70">
            ...
            <Link href="/plan" className="flex items-center gap-1 rounded-full bg-[var(--green)] px-4 py-1.5 font-bold text-white transition hover:bg-[var(--green-dark)]">
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <input
                      ...
                      placeholder="כל מקום"
                      className="w-full bg-transparent text-sm font-bold outline-none placeholder:font-bold placeholder:text-[var(--ink)]"
                    />
                  </span>
                  <button
                    onClick={(e) => {
                ...
                <div className="p-4">
                  ...
                  {children > 0 && (
                    <div className="mt-2 border-t border-[var(--line)] pt-2.5">
                      <div className="mb-2 text-xs font-bold text-[var(--muted)]">גיל הילדים בזמן הנסיעה</div>
                  ...
                  <button onClick={() => setOpenField(null)} className="mt-3 w-full rounded-xl bg-[var(--green)] py-2 text-sm font-black text-white transition hover:bg-[var(--green-dark)]">אישור</button>
                </div>
              </FieldShell>

              <ParticleButton className="h-auto rounded-[1.4rem] bg-[var(--green)] px-8 py-4 text-base font-black text-white shadow-lg shadow-[var(--green)]/20 hover:bg-[var(--green-dark)]">
                חפש
              </ParticleButton>
            </div>
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-2xl font-black">תוצאות מומלצות עבורך</h2>
              <Sparkles className="h-5 w-5 text-amber-400" />
            </div>
            <p className="mt-1 text-sm text-[var(--muted)]">נבחרו במיוחד לפי ההעדפות שלך</p>
          </div>
          <button className="flex items-center gap-2 rounded-xl border border-[var(--line)] bg-white px-4 py-2 text-sm font-bold shadow-sm">
        ...
        <div className="grid gap-5 lg:grid-cols-[15rem_1fr_14rem]">
          <aside className="h-fit rounded-2xl border border-[var(--line)] bg-white p-4 shadow-sm">
            <div className="flex items-center justify-between">
              <div className="text-base font-black">סינון תוצאות</div>
            ...
            <div className="mt-3 border-t border-[var(--line)] py-3">
              <div className="mb-2 text-sm font-bold">דירוג אורחים</div>
              <div className="flex flex-wrap gap-1.5">
                {[{ id: "9", label: "9+" }, { id: "8", label: "8+" }, { id: "7", label: "7+" }, { id: "all", label: "כל הדירוגים" }].map((r) => (
                  <button key={r.id} onClick={() => setRating(r.id)}
                    className={`rounded-full border px-3 py-1 text-xs font-bold transition ${rating === r.id ? "border-[var(--green)] bg-[var(--green)] text-white" : "border-[var(--line)] text-[var(--ink)]/70 hover:border-[var(--green)]/50"}`}>
                    {r.label}
                  </button>
                ))}
              </div>
            ...
            <button className="mt-4 w-full rounded-xl bg-[var(--green)] py-2.5 text-sm font-black text-white transition hover:bg-[var(--green-dark)]">
              הצג 2,842 תוצאות
            </button>
          </aside>
          ...
          <aside className="h-fit overflow-hidden rounded-2xl border border-[var(--line)] bg-white text-center shadow-sm">
            <div className="bg-gradient-to-b from-[var(--sand)] to-white p-5">
              ...
              <div className="text-base font-black">נשמח להכיר אתכם</div>
            ...
            <div className="p-4 pt-0">
              <Link href="/plan" className="flex w-full items-center justify-center gap-1.5 rounded-xl bg-[var(--green)] px-4 py-2.5 text-sm font-black text-white transition hover:bg-[var(--green-dark)]">
      ...
      <section className="mx-auto max-w-7xl px-4 pb-14">
        <div className="mb-5">
          <div className="flex items-center gap-2">
            <h2 className="text-2xl font-black">יעדים שאוהבים</h2>
            <Sparkles className="h-5 w-5 text-amber-400" />
          </div>
  ...
  return (
    <div className="group flex flex-col overflow-hidden rounded-2xl border border-[var(--line)] bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">
      <div className="relative h-40 overflow-hidden bg-gradient-to-br from-stone-200 to-stone-300">
        ...
        {h.badge && <span className="absolute right-3 top-3 rounded-full bg-[var(--green)] px-2.5 py-1 text-xs font-black text-white shadow">{h.badge}</span>}
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mt-1 text-base font-black leading-tight">{h.name}</div>
        <div className="mt-1 flex items-center gap-1 text-xs text-[var(--muted)]">
          <MapPin className="h-3.5 w-3.5" /> {h.city}
        </div>

        <div className="mt-2 flex items-center gap-2">
          <span className="rounded-lg bg-[var(--green)] px-2 py-0.5 text-sm font-black text-white">{h.rating}</span>
        ...
        <div className="mt-3 rounded-xl bg-[var(--bg)] px-3 py-2 text-xs">
          <div className="flex items-center gap-1.5 font-bold"><Plane className="h-4 w-4 text-[var(--green)]" /> {h.flight.date}</div>
        ...
        <div className="mt-4 flex items-end justify-between border-t border-dashed border-[var(--line)] pt-3">
          <div>
            <div className="text-2xl font-black text-[var(--green)]">${h.price}</div>
            <div className="text-[11px] text-[var(--muted)]">לאדם</div>
          </div>
          <button className="rounded-xl border border-[var(--green)] px-4 py-2 text-sm font-black text-[var(--green)] transition hover:bg-[var(--green)] hover:text-white">
            צפה בהצעה
          </button>
        </div>
      </div>
